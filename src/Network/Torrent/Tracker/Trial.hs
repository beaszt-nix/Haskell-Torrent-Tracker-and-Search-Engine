...
module Network.Torrent.Tracker.Trial where
...
type ReqAPI = "announce" 
            :> QueryParam "info_hash" Word160 
            :> QueryParam "peer_id" Word160
...
type ScrapeAPI = "scrape" :> QueryParams "info_hash" Word160 :> Servant.Get '[PlainText] BEncode 
...
scrapeHandler :: AnnounceEnv -> [ScrapeReq] -> Handler BEncode
...
instance FromHttpApiData Word160 where
  parseUrlPiece x = do
    let s  = URI.urlDecode False . toStrict . toLazyByteString . encodeUtf8Builder $ x 
    if (B8.length s) /= 20
      then Left $ T.pack "Not enough bytes"
      else return $ Bin.decode $ fromStrict s 
...
parseEvent = let    cmp = string (B8.pack "completed") *> pure Completed
                  ...
                  in  either (\_ -> Nothing) (\x -> Just x) . parseOnly (cmp <|> str <|> stp) . B8.pack . fromMaybe ""
